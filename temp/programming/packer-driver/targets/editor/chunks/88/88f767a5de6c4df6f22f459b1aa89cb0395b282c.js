        ...
        static optDc(scrollView, caculatePosition) {
          ...
          if (uiTransform) {
            ...
            if (parentTransform) {
              ...
              let svLeftBottomPoint = parentTransform.convertToWorldSpaceAR(new Vec3(scrollViewPosition.x - uiTransform.anchorX * uiTransform.width, scrollViewPosition.y - uiTransform.anchorY * uiTransform.height, scrollViewPosition.z)); // 求出 ScrollView 可视区域在世界坐标系中的矩形（碰撞盒）

              let svBBoxRect = new Rect(svLeftBottomPoint.x, svLeftBottomPoint.y, uiTransform.width, uiTransform.height); // 遍历 ScrollView Content 内容节点的子节点

              scrollView.content.children.forEach(childNode => {
                // 获取子节点的 UITransform 组件
                let childTransform = childNode.getComponent(UITransform); // 没有绑定指定组件的子节点不处理

                let itemComponent = childNode.getComponent(_crd && ScrollViewPlusItem === void 0 ? (_reportPossibleCrUseOfScrollViewPlusItem({
                  error: Error()
...
//# sourceMappingURL=88f767a5de6c4df6f22f459b1aa89cb0395b282c.js.map